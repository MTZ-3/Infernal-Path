...
const lvl = card.level || 1; const e = card.effect || {base:0,growth:0,scaleType:'linear'};
if(e.scaleType==='log'){
const cap = e.cap ?? Infinity; const val = e.base + Math.log1p(Math.max(0,lvl-1))*e.growth; return Math.min(val, cap);
}
...
const h = GameState.hero; const val = scaledValue(card);
const isCurse = card.type==='fluch';
const dmgMod = 1 + (isCurse? GameState.mods.cursePct/100 : 0) + (GameState.mods.tempDamagePct/100);


const finish = ()=>{ GameState.discard.push(card); GameState.hand = GameState.hand.filter(c=>c.uid!==card.uid); };
...
if(k==='freeze_days'){ const d=Math.max(1,Math.round(val)); h.status.frozenDays+=d; finish(); return {ok:true, log:`${card.name}: Eingefroren ${d}T.`}; }
if(k==='slow_move_days'){ const d=Math.max(1,Math.round(val)); h.status.slowDays+=d; finish(); return {ok:true, log:`${card.name}: Bewegung -1 für ${d}T.`}; }
...
if(k==='weaken'){ const d=Math.max(1,Math.round(val)); h.status.weakenPct=clamp(h.status.weakenPct+d,0,90); finish(); return {ok:true, log:`${card.name}: Schwächung +${d}%.`}; }
...
if(k==='extend_run_days'){ const d=Math.max(1,Math.round(val)); import("../../core/gameState.js").then(m=>m.GameState.extendDays(d)); finish(); return {ok:true, log:`${card.name}: +${d} Tage.`}; }
...
// Platzhalter für Map-/Eroberungs-/Beschwörungsarten: Wir akzeptieren die Platzierung, loggen Ziel
finish();
return {ok:true, log:`${card.name} wurde auf Feld ${targetNodeId??'?'} platziert.`};
}
...
export function reshuffleIfNeeded(){ if(GameState.deck.length===0 && GameState.discard.length>0){ GameState.deck = shuffle(GameState.discard); GameState.discard=[]; log(`Ablagestapel wird gemischt.`);} }
